...
int choice_judge() {//读取键盘事件
	...
	while(true){
		...
		if (event.EventType == KEY_EVENT)      //如果当前事件是键盘事件  
		{
			...
			{
				//screen_clear(highlight==1?highlight=3:--highlight);//i 控制高亮
			...
			{
				//screen_clear(highlight == 3 ? highlight = 1:++highlight);//i 控制高亮
				highlight = screen_clear(1, highlight, 1);
			}
