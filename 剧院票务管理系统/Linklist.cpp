...
void sale_ticket(Plan *p, int judge) {//售票   //结束后刷新key  judge  用于不同的端 0售票员 1顾客
...
//	for (int i = 1; i <= (x - 1) *(s->element.seaty) + y - 1; i++) {
	...
	//printf("%d  %d,%d %d\n",x,y, t->seatx, t->seaty);
	if(t){
		judge == 1 ? printf("是否确认购买(0/1)?:") : printf("是否确认售出(0/1)?:");
	if (get_num(0, 1, 1, 1)) {
...
void return_ticket(Ticket *t, int judge) {//退票    并产生销售记录 judge控制端口  0售票员 1顾客
	if (t) {
		if (judge) {//判断票的持有者
...
void draw_ticket(Ticket *t, int judge) {//画一张票//  0 售票员 1剧院经理
	Plan *p = search_plan(t->plan_ID, 0, list.plan_head);
	if(judge && p==NULL)p= search_plan(t->plan_ID, 0, list.plan_tem_head);
	draw_ticket(p, t);
}

Ticket *search_ticket(long ID ,Plan *head) {//根据ID查找票  head 查找范围 judge控制相似反馈
	Plan *p = head->next; int flag  = 0;
	while (p->element.ticket_tail->ticket_ID < ID)p = p->next;//定位到演出计划
	Ticket *t = p->element.ticket_head->next;
	for (t; t; t = t->next) {
		if (t->ticket_ID == ID)
			flag = 1; break;
...
Record  *add_record(Plan *p, Ticket *t,Account *a, sale_types type) {//产生一条销售记录加到记录链表尾  更新主键并刷新业绩
	timer();
